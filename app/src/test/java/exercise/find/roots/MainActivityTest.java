...
public class MainActivityTest extends TestCase {
    ...
    public void when_userIsEnteringNumberInput_and_noCalculationAlreadyHappened_then_theButtonShouldBeEnabled() {
        ...
        inputEditText.setText("99");
        assertTrue(button.isEnabled());
    }

    @Test
    public void when_activityIsLaunching_then_theProgressStartHidden() {
        // create a MainActivity and let it think it's currently displayed on the screen
        MainActivity mainActivity = Robolectric.buildActivity(MainActivity.class).create().visible().get();
        ...
        // test: check that progress bar is hidden
        assertEquals(progressBar.getVisibility(), View.GONE);
    }

    @Test
    public void when_userIsEnteringBadInput_and_noCalculationAlreadyHappened_then_theButtonShouldBeDisabled() {
        // create a MainActivity and let it think it's currently displayed on the screen
        MainActivity mainActivity = Robolectric.buildActivity(MainActivity.class).create().visible().get();
